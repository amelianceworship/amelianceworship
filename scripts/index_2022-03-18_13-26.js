...
const transposeKey = (chords, transpose) => {
    ...
    for (let line of transposedChordsArray) {
             let max = line.reduce((value, item) => {
                return item.length > value ? item.length : value;
             }, 0);
    ...
    for (let line of transposedChordsArray) {
        ...
        // Msg(tempLine)
        transposedChords.push(tempLine);
    }
